...
fun BenoitScreen(benoitViewModel: BenoitViewModel = viewModel()){
    ...
    BenoitLayout(benoitViewModel = benoitViewModel, currentTitle = benoitStateUI.title)
}

@Composable
fun BenoitLayout(benoitViewModel: BenoitViewModel, currentTitle: String ){
    val textPostState = remember { mutableStateOf(TextFieldValue()) }
    val textXPState = remember {
    ...
    Box(
        ...
        Column(
            ...
            TextField(
                ...
                Button(onClick = { benoitViewModel.makeRequest(textPostState.value.text, textXPState.value.text) }, modifier = Modifier.padding(bottom = 90.dp)) {
